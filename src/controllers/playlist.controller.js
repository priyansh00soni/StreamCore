...
const getUserPlaylists = asyncHandler(async(req,res)=>{
    ...
    if (!playlists || playlists.length === 0) throw new ApiError(404, "Playlist not found");
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    if(req.user._id.toString() !== playlistOwner.toString()) throw new ApiError(403,"Only Owner is allowed to edit their playlist")
...
const updatePlaylist = asyncHandler(async (req, res) => {
    ...
    if(!name && !description) throw new ApiError(400,"Updated atleast one field.")
    
    const playlist = await Playlist.findById(playlistId)
    if(!playlist) throw new ApiError(404, "Playlist not found.")
    if(playlist.owner.toString() !== req.user._id.toString()) throw new ApiError(403, "Forbidden.")
