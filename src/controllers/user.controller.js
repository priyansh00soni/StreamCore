...
const registerUser = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(
        //createdUser would also work but we need structure and thats why we made that utility.
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const options = {
        //`httpOnly: true` means JavaScript cannot read OR modify the cookie. Only the browser sends it automatically with requests. Frontend JS has zero access.
...
const updateUserAvatar = asyncHandler(async (req, res) => {
    ...
    if (!(await deleteFromCloudinary(avatarPublicId)))
        throw new ApiError(400, 'Avatar deletion failed.')
    const user = await User.findByIdAndUpdate(
        req.user._id,
        {
            $set: {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, user, 'Avatar updated Scuccessfully.'))
})
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
    ...
    const coverImagePublicId = req.user.coverImage
        ...
        .at(-1)
        .split('.')[0]
    if (!(await deleteFromCloudinary(coverImagePublicId)))
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, user, 'Cover Image updated Scuccessfully.'))
})
...
const getChannelProfile = asyncHandler(async (req, res) => {
    ...
    const channel = await User.aggregate([
        ...
        {
            $project: {
                //we will provide selected things.
                fullName: 1,
                username: 1,
                subscribersCount: 1,
